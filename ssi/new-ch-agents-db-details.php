...
$sql809 = "SELECT * FROM ch_agents_2 WHERE Website='".$actual_link."' LIMIT 1";
...
//Check if this agent has assigned partners other than non-paying sub-agents=====================
$sql09003 = "SELECT * FROM ch_agents, ch_partner_connections WHERE ch_agents.AgentID='".$AgentID."' and ch_agents.AgentID=ch_partner_connections.AgentID and ch_partner_connections.active=1 and ch_agents.AgentType !='Non-Paying Sub-Agent'";
...
$sql99999 = "SELECT * FROM ch_agents WHERE AgentID ='".$PartnerID0900."' AND (showsubagents=1 || showsubagents='') AND AgentType !='Non-Paying Sub-Agent' LIMIT 1";
...
if (empty($AgentID))
$NewAboutMetaDescription='Centre Holidays is a host travel agency consisting of over 250 home based travel agents who help travellers on a daily basis get away on their dream trip.';
...
<br>
<br>
Since 2010, Centre Holidays has grown from a 2-person organization to having over 250 home based travel agents across Canada.
<br>
<br>
...
if (!empty($Slider2Image)){
$Slider2Image='https://centreholidays.com/img/agents/'.$AgentID.'/'.$Slider2Image;
//https://centreholidays.com/img/agents/634/index-c02-havana-cuba.jpg
}

if (!empty($Slider3Image)){
$Slider3Image='https://centreholidays.com/img/agents/'.$AgentID.'/'.$Slider3Image;
}
...
$FrontYouTube = $FrontFaceBook = $FrontTwitter = $FrontLinkedIn =
$FrontPinterest = $FrontInstagram = $FrontTikTok = $FrontBlog = "";
...
if (empty($AgentID)) {
	...
	if (!empty($YouTube)) $FrontYouTube = 'https://youtube.com/c/' . $YouTube;

	if (!empty($FaceBook)) $FrontFaceBook = 'https://facebook.com/' . $FaceBook;

	if (!empty($Twitter)) $FrontTwitter = 'https://twitter.com/' . $Twitter;

	if (!empty($LinkedIn)) $FrontLinkedIn = 'https://linkedin.com/' . $LinkedIn;

	if (!empty($Pinterest)) $FrontPinterest = 'https://pinterest.com/' . $Pinterest;

	if (!empty($Instagram)) $FrontInstagram = 'https://instagram.com/' . $Instagram;

	if (!empty($TikTok)) $FrontTikTok = 'https://www.tiktok.com/' . $TikTok;
