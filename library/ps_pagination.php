...
class PS_Pagination {
	 ...
	 * @return string
	 */
	function renderNext($tag = '<i class="fa fa-angle-right"></i>') {
		if ($this->total_rows == 0)
			return FALSE;
	 ...
	 * @return string
	 */
	function renderPrev($tag = '<i class="fa fa-angle-left"></i>') {
		if ($this->total_rows == 0)
			return FALSE;
	 ...
	 * @return string
	 */
	function renderNav($prefix = '<span class="">', $suffix = '</span>') {
		if ($this->total_rows == 0)
			return FALSE;
		...
		for($i = $start; $i <= $end; $i ++) {
			if ($i == $this->page) {
				$links .=  '<li class="active">'.$prefix .$i.$suffix.'</li>';
			} else {
