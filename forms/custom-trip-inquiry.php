...
//echo "<pre>"; print_r($response_keys); echo "</pre>";
//exit;
//GP ADDED -- END....
...
//#########  Captcha End   #########
    ...
    $to = 'krunalpatel5390@gmail.com';
    $headers = 'From: krunal@geekpower.ca' . "\r\n" .
        'Reply-To: krunal@geekpower.ca' . "\r\n" .
        'X-Mailer: PHP/' . phpversion();

    $subject = 'Custom Trip Inquiry -- GOOGLE CAPTCHA -- SUCCESS TEST';
    $message = "Custom Trip Inquiry -- GOOGLE CAPTCHA -- SUCCESS TEST ==============<hr> $string_output";
    // Send email
    ...
    if($response_keys['score'] <= 0.8){

        $string_output = print_r($_POST, true);

        $to = 'krunalpatel5390@gmail.com';
        $subject = 'Custom Trip Inquiry less than 0.8 captcha..';
        $message = 'Custom Trip Inquiry'.$response_keys['score']." ==============<hr> $string_output";
        $headers = 'From: krunal@geekpower.ca' . "\r\n" .
            'Reply-To: krunal@geekpower.ca' . "\r\n" .
            'X-Mailer: PHP/' . phpversion();
...
//Need to keep elseif and not else or you will get email on just page load
elseif (!empty($travfname) && !empty($travlname)) {
...
//    $subject1 = 'Custom Trip Inquiry -- GOOGLE CAPTCHA ERROR - EMAIL 1';
//    $message1 = "Custom Trip Inquiry -- GOOGLE CAPTCHA ERROR - EMAIL 1";
//    // Send email
//    if (mail($to, $subject1, $message1, $headers)) {
//        echo 'Email sent successfully.';
//    }

    $subject2 = 'Custom Trip Inquiry -- GOOGLE CAPTCHA ERROR- EMAIL 2 ';
    $message2 = "Custom Trip Inquiry -- GOOGLE CAPTCHA ERROR ==============<hr> Name: $travfname -- $travlname
         <hr>
         $string_output";
