...
/*	//============FILTER DATABSE BASED ON selected values====================	
	...
	{
	$sql .=" AND (promooperators LIKE '%".$AgentInterest1."%' || promooperators LIKE '%".$AgentInterest2."%' || promooperators LIKE '%".$AgentInterest3."%' || promooperators LIKE '%".$AgentInterest4."%' || promooperators LIKE '%".$AgentInterest5."%' || promooperators LIKE '%".$AgentInterest6."%' || promooperators LIKE '%".$AgentInterest7."%' || promooperators LIKE '%".$AgentInterest8."%' || promooperators LIKE '%".$AgentInterest9."%' || promooperators LIKE '%".$AgentInterest10."%' || promooperators LIKE '%".$AgentInterest11."%' || promooperators LIKE '%".$AgentInterest12."%') ";
	$sql .=" AND (promosinterests LIKE '%".$AgentInterest1."%' || promosinterests LIKE '%".$AgentInterest2."%' || promosinterests LIKE '%".$AgentInterest3."%' || promosinterests LIKE '%".$AgentInterest4."%' || promosinterests LIKE '%".$AgentInterest5."%' || promosinterests LIKE '%".$AgentInterest6."%' || promosinterests LIKE '%".$AgentInterest7."%' || promosinterests LIKE '%".$AgentInterest8."%' || promosinterests LIKE '%".$AgentInterest9."%' || promosinterests LIKE '%".$AgentInterest10."%' || promosinterests LIKE '%".$AgentInterest11."%' || promosinterests LIKE '%".$AgentInterest12."%') ";	
	}
*/
...
/*	$sql .=" promosinterests LIKE '%".$AgentInterest1."%' DESC, promocontinent LIKE '%".$AgentInterest1."%' DESC, promocountry LIKE '%".$AgentInterest1."%' DESC, promooperators LIKE '%".$AgentInterest1."%' DESC, ";
	...
	$sql .=" promosinterests LIKE '%".$AgentInterest12."%' DESC, promocontinent LIKE '%".$AgentInterest12."%' DESC, promocountry LIKE '%".$AgentInterest12."%' DESC, promooperators LIKE '%".$AgentInterest12."%' DESC, ";		
		*/
		//$sql .= " promopintop DESC, promotimestamp DESC";
		//}
		//else
...
$AgentID2=str_replace(',', "", $row['AgentID']);
	...
	//=========================get the agent URL====
	//$sql717 = "SELECT * FROM ch_agents WHERE AgentID='".$AgentID2."' and active=1 LIMIT 1";
	$sql717 = "SELECT * FROM ch_agents_1, ch_agents_2 WHERE (ch_agents_1.AgentID='".$AgentID2."' AND ch_agents_2.AgentID='".$AgentID2."') and ch_agents_1.active=1 LIMIT 1";
	$result717 = mysqli_query($conn, $sql717);
	// output data of each row
	while($row717 = mysqli_fetch_assoc($result717)) {
	
	$PAgentWebsite=trim($row717['Website']); //	
	
	if (!empty($PAgentWebsite))// If this highlight displayed in Corp. website link it to the agent URL
	$PWebsite=$HAgentWebsite.'/';	
...
<!--begin::Offer-->
<div class="card card-border card-custom mb-8 mb-lg-0 card-stretch">
<!--begin::Ribbon-->
...
<h2 class="font-weight-bolder"><?php echo $row['promotitle']; ?></h2>														
<p><?php echo $row['promodetails']; ?></p>
<?php $promodepartfrom=trim($row['promodepartfrom']); ?>
<small class="font-size-xs">Valid till <?php echo date('F j, Y', strtotime($row['promoexpirydate'])); ?> • <?php echo rtrim($promodepartfrom, ','); ?></small>
</div>

<div class="mt-10 d-flex">
<a href="<?php echo 'https://'.$PWebsite ?>special-offers/featured-offer.php?pro=<?php echo $row['promourl']; ?>&promocode=<?php echo $row['promocode']; ?>" title="View Special Offer" class="btn btn-warning px-5 py-3 text-uppercase font-weight-bolder">
See Special Offer
</a>
...
<button type="button" class="btn btn-warning font-weight-bolder px-5 py-3 text-uppercase btn-lg mb-10 mt-5" id="load-more-button-offers" title="See More">See More Special Offers</button>
</div>
