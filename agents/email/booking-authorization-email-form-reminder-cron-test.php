...
$sql2 = "SELECT * FROM ch_agents_1 ORDER BY AgentID ASC";
...
$sql = "SELECT * FROM booking_1 WHERE FormCancelled='' AND AgentID='".$AgentID."' AND FormApproved='' AND BookingTime > $ptime AND (BookingStatus=3 || BookingStatus=4) ORDER BY BookID DESC";
...
$urllink='https://centreholidays.com/agents/booking-authorization-email-form-preview.php?AgentID='.$AgentID.'&BookID='.$BookID.'&allow=no&agentcron=0&BusinessFullName='.$BusinessFullName;
...
<title>Booking Authorization Email Form Reminder | Centre Holidays</title>
</head>
...
<tr>
  ...
  <tr>
    <td><p>Dear '.$BusinessFullName.',<br />
      <br />
      Your client ('.$PTravellerFullName.') has filled in the booking authorization email form. Please go over the details provided and take the next steps accordingly. <b>Please be vigilant so that a fraudulent booking does not take place.</b><br />
<br />
If you have any questions in regards to the email form or notice any bugs, please email <b>support@centreholidays.com</b>.</p>
      <center>Booking ID: '.$BookingID.'<br />
